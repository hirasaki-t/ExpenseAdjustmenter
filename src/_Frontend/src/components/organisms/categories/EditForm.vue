...
const category = categoryStore.dictionary[props.id];
...
async function update() {
    ...
    formRef.value.validate(async (valid) => {
        if (valid) {
            loading.value = true;
            try {
                await categoryStore.update(props.id, formValue!.name, formValue!.details, formValue!.isReceipt, formValue!.isActive);
                emit("complete");
            } finally {
                loading.value = false;
            }
