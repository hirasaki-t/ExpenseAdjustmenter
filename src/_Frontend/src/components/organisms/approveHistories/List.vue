<template>
    <el-table :data="approveHistories" border show-summary :summary-method="getSummaries" @selection-change="selected" height="100%">
            ...
            <template #header>
                <custom-filter title="氏名" width="80px">
                    <el-select v-model="userName" clearable placeholder="氏名">
                        <el-option v-for="userName in userNames " :label="(userName as string)" :value="(userName as string)"/>
                    </el-select>
                </custom-filter>
            </template>
            <template #default="scope">
            ...
            <template #header>
                <custom-filter title="分類" width="100px">
                    <el-select v-model="type" clearable placeholder="分類">
                        <el-option v-for="expenseType in types" :label="(expenseType as string)" :value="(expenseType as string)"/>
                    </el-select>
                </custom-filter>
            </template>
        </el-table-column>
        ...
        <el-table-column align="center" width="100">
            <template #header>
                <custom-filter title="ステータス" width="100px">
                    <status-selector v-model="status" clearable />
                </custom-filter>
            </template>
            <template #default="scope">
        ...
        <details-form v-if="detailsFormShow" :id="selectedRowId" :show="detailsFormShow" @complete="detailsFormShow = false" />
        <history-dialog v-if="historyFormShow" :id="selectedRowId" :show="historyFormShow" @complete="historyFormShow = false" />
    </el-table>
</template>
...
const approveHistories = computed(() => approveHistoryStore.orderedApproveHistories
...
const userNames = computed(() => distinct(approveHistoryStore.approveHistories.map(x => userDictionary[x.userId]?.name)));
const types = computed(() => distinct(approveHistoryStore.approveHistories.map(x => x.type)));
const dates = computed(() => distinct(approveHistoryStore.approveHistories.map(x => x.date.getTime()).sort((x, y) => x - y))?.map(x => new Date((x as number))));
...
function selected(approveHistories: ApproveHistory[]) {
    emit("update:modelValue", approveHistories.map(x => x.expenseId));
}
</script>
