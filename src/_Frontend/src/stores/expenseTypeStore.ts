...
import { ExpenseType } from "../datas/expenseType";
...
export const useExpenseTypeStore = defineStore("expenseType", () => {
    ...
    const dictionary = computed(() => toDictionary(orderedExpenseTypes.value, x => x.id, x => x));

    async function initialize() {
        expenseTypes.value = await requestGetAll();
    }

    async function add(name: string, details: string, isReceipt: boolean){
