...
import type { ApproveHistory } from "@/types";

export const useApproveHistoryStore = defineStore("approveHistory", () => {
  const approveHistories = ref<ApproveHistory[]>([]);
  const orderedApproveHistories = computed(() =>
    Array.from(approveHistories.value).sort((x, y) => dateSorter(x.date, y.date))
  );
  const dictionary = computed(() =>
    toDictionary(
      approveHistories.value,
      (x) => x.id,
      (x) => x
    )
  );
  ...
  async function initialize() {
    ...
    approveHistories.value = selectableMonths.value.some(
      (x) => x.getFullYear() === thisMonth.getFullYear() && x.getMonth() === thisMonth.getMonth()
    )
      ? await requestGets(thisMonth.getFullYear(), thisMonth.getMonth() + 1)
      : await requestGets(addMonths(thisMonth, -1).getFullYear(), thisMonth.getMonth());
  }
